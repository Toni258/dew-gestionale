...
function buildWhereArchive({ start, end, meal, patientId, floor, course }) {
    let where = ` WHERE 1=1 `;
    const params = [];
...
function buildWhereChoiceArchive({
    start,
    end,
    meal,
    patientId,
...
export async function getConsumiReportArchive(req, res) {
    try {
        const {
            start = '',
            end = '',
        ...
        // paziente versionato: prendiamo riga più recente (come nel tuo live)
        const latestPatientJoin = `
            JOIN (
                SELECT id_patient, MAX(last_changed) AS last_changed
                FROM patient
        ...
        const { where, params } = buildWhereArchive({
            start,
            end,
            meal,
            patientId,
        ...
        const { where: whereChoice, params: paramsChoice } =
            buildWhereChoiceArchive({
                start,
                end,
                meal,
                patientId,
        ...
        console.error('Errore getConsumiReportArchive:', err);
        return res.status(500).json({ error: 'Errore interno al server' });
    }
}
