...
export async function getArchivedMenuFixedCheesesRotation(idArchMenuRaw) {
    ...
    // normalizzo endpoint: { pranzo:[...], cena:[...] }
    const out = { pranzo: Array(7).fill(null), cena: Array(7).fill(null) };

    for (const r of rows) {
        const meal = r.meal_type; // 'pranzo'|'cena'
        const idx = Number(r.day_index);
        if (!out[meal]) continue;
        if (idx < 0 || idx > 6) continue;

        out[meal][idx] = { id_food: r.id_food, name: r.name };
    }
